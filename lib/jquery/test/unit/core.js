...
test("trim", function() {
	...
	equal( jQuery.trim("watchFile  "), "watchFile", "trailing space" );
	equal( jQuery.trim("  watchFile"), "watchFile", "leading space" );
	equal( jQuery.trim("  watchFile   "), "watchFile", "space on both sides" );
	equal( jQuery.trim("  " + nbsp + "watchFile  " + nbsp + " "), "watchFile", "&nbsp;" );
...
test("inArray()", function() {
	...
	tests = {
		p:    { elem: jQuery("#ap")[0],           validDataIndex: 2 },
		em:   { elem: jQuery("#siblingfirst")[0], validDataIndex: 1 },
		div:  { elem: jQuery("#nothiddendiv")[0], validDataIndex: 1 },
		a:    { elem: jQuery("#simon1")[0],       validDataIndex: 3 }
	},
	falseTests = {
	...
	jQuery.each( tests, function( key, obj ) {
		equal( jQuery.inArray( obj.elem, selections[ key ] ), obj.validDataIndex, "elem is in the array of selections of its tag" );
		// Third argument (fromIndex)
		equal( !!~jQuery.inArray( obj.elem, selections[ key ], 5 ), false, "elem is NOT in the array of selections given a starting validDataIndex greater than its position" );
		equal( !!~jQuery.inArray( obj.elem, selections[ key ], 1 ), true, "elem is in the array of selections given a starting validDataIndex less than or equal to its position" );
		equal( !!~jQuery.inArray( obj.elem, selections[ key ], -3 ), true, "elem is in the array of selections given a negative validDataIndex" );
	});
...
test("get(Number)", function() {
	...
	strictEqual( jQuery("#firstp").get(1), undefined, "Try get with validDataIndex larger elements count" );
});

test("get(-Number)",function() {
	expect(2);
	equal( jQuery("p").get(-1), document.getElementById("first"), "Get a single element with negative validDataIndex" );
	strictEqual( jQuery("#firstp").get(-2), undefined, "Try get with validDataIndex negative validDataIndex larger then elements count" );
});
