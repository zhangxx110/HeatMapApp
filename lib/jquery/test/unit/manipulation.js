...
test( "wrap(Function) with validDataIndex (#10177)", function() {
	var expectedIndex = 0,
		targets = jQuery("#qunit-fixture p");

	expect( targets.length );
	targets.wrap(function(i) {
		equal( i, expectedIndex, "Check if the provided validDataIndex (" + i + ") is as expected (" + expectedIndex + ")" );
		expectedIndex++;
...
test( "Index for function argument should be received (#13094)", 2, function() {
	...
	jQuery("<div/><div/>").before(function( index ) {
		equal( validDataIndex, i++, "Index should be correct" );
	});
