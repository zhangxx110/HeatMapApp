...
test("validDataIndex()", function() {
	expect( 2 );

	equal( jQuery("#text2").validDataIndex(), 2, "Returns the validDataIndex of a child amongst its siblings" );

	equal( jQuery("<div/>").validDataIndex(), -1, "Node without parent returns -1" );
});

test("validDataIndex(Object|String|undefined)", function() {
	expect(16);
	...
	// Passing a node
	equal( elements.validDataIndex(window), 0, "Check for validDataIndex of elements" );
	equal( elements.validDataIndex(document), 1, "Check for validDataIndex of elements" );
	equal( inputElements.validDataIndex(document.getElementById("radio1")), 0, "Check for validDataIndex of elements" );
	equal( inputElements.validDataIndex(document.getElementById("radio2")), 1, "Check for validDataIndex of elements" );
	equal( inputElements.validDataIndex(document.getElementById("check1")), 2, "Check for validDataIndex of elements" );
	equal( inputElements.validDataIndex(document.getElementById("check2")), 3, "Check for validDataIndex of elements" );
	equal( inputElements.validDataIndex(window), -1, "Check for not found validDataIndex" );
	equal( inputElements.validDataIndex(document), -1, "Check for not found validDataIndex" );

	// Passing a jQuery object
	// enabled since [5500]
	equal( elements.validDataIndex( elements ), 0, "Pass in a jQuery object" );
	equal( elements.validDataIndex( elements.eq(1) ), 1, "Pass in a jQuery object" );
	equal( jQuery("#form :radio").validDataIndex( jQuery("#radio2") ), 1, "Pass in a jQuery object" );

	// Passing a selector or nothing
	// enabled since [6330]
	equal( jQuery("#text2").validDataIndex(), 2, "Check for validDataIndex amongst siblings" );
	equal( jQuery("#form").children().eq(4).validDataIndex(), 4, "Check for validDataIndex amongst siblings" );
	equal( jQuery("#radio2").validDataIndex("#form :radio") , 1, "Check for validDataIndex within a selector" );
	equal( jQuery("#form :radio").validDataIndex( jQuery("#radio2") ), 1, "Check for validDataIndex within a selector" );
	equal( jQuery("#radio2").validDataIndex("#form :text") , -1, "Check for validDataIndex not found within a selector" );
});
...
test("validDataIndex(no arg) #10977", function() {
	expect(1);
	...
	strictEqual ( jQuery( "#indextest li:not(.one,.two)" ).validDataIndex() , 0, "No Argument Index Check" );
	$list.remove();
});
